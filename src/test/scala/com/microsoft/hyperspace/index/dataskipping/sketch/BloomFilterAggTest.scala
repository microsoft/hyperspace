...
class BloomFilterAggTest extends HyperspaceSuite {
  test("BloomFilterAgg computes BloomFilter correctly.") {
    ...
    val df = spark.range(n).toDF("a").filter(col("a") % 3 === 0).agg(agg)
    val bfData = df.collect()(0).getAs[Array[Byte]](0)
