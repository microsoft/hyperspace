...
object JoinNodeCheck extends HyperspacePlanCheck {
  override def apply(
      plan: LogicalPlan,
    ...
    plan match {
      case Join(l, r, _, Some(condition))
          if isJoinConditionSupported(condition) && isPlanLinear(l) && isPlanLinear(r) =>
        ...
        if (left.isDefined && right.isDefined && !RuleUtils.isIndexApplied(left.get) && !RuleUtils
          ...
          JoinIndexBatch.leftRelation.set(left.get)
          JoinIndexBatch.rightRelation.set(right.get)
          JoinIndexBatch.joinCondition.set(condition)
          indexes.filterKeys(planKey =>
            left.get.plan.equals(planKey) || right.get.plan.equals(planKey))
        } else {
          Map.empty
        }
      case _ =>
  ...
  override def reason: String = {
    "JoinIndex: no applicable join node or join condition."
  }
}

object JoinAttributeCheck extends HyperspacePlanCheck {
  override def apply(
      plan: LogicalPlan,
      indexes: Map[LogicalPlan, Seq[IndexLogEntry]]): Map[LogicalPlan, Seq[IndexLogEntry]] = {
    if (indexes.isEmpty) {
    ...
    if (ensureAttributeRequirements(
          JoinIndexBatch.leftRelation.get,
          JoinIndexBatch.rightRelation.get,
          JoinIndexBatch.joinCondition.get)) {
      indexes
    } else {
      Map.empty
    }
  ...
  private def ensureAttributeRequirements(
    ...
    JoinIndexBatch.extractConditions(condition).forall {
      case EqualTo(e1, e2) =>
  ...
  override def reason: String = {
    // TODO revise
    "JoinIndex: no applicable join condition. Each join condition column should come from " +
      "relations directly and attributes from left subplan must exclusively have " +
      "one-to-one mapping with join condition attributes from right subplan."
  }
}

object JoinColumnCheck extends HyperspacePlanCheck {
  override def apply(
      plan: LogicalPlan,
      indexes: Map[LogicalPlan, Seq[IndexLogEntry]]): Map[LogicalPlan, Seq[IndexLogEntry]] = {
    if (indexes.isEmpty) {
    ...
    val leftRelation = JoinIndexBatch.leftRelation.get
    val rightRelation = JoinIndexBatch.rightRelation.get
    ...
    val lRMap =
      JoinIndexBatch.getLRColumnMapping(lBaseAttrs, rBaseAttrs, JoinIndexBatch.joinCondition.get)
    ...
    plan match {
      case Join(l, r, _, _) =>
        ...
        val lIndexes =
          getUsableIndexes(
            indexes.getOrElse(leftRelation.plan, Seq()),
            lRequiredIndexedCols,
            lRequiredAllCols)
        val rIndexes =
          getUsableIndexes(
            indexes.getOrElse(rightRelation.plan, Seq()),
  ...
  override def reason: String = "JoinIndex: output column does not match."
}

object JoinRankCheck extends HyperspacePlanCheck {
  override def apply(
      plan: LogicalPlan,
      indexes: Map[LogicalPlan, Seq[IndexLogEntry]]): Map[LogicalPlan, Seq[IndexLogEntry]] = {
    if (indexes.isEmpty) {
    ...
    val lRMap =
      JoinIndexBatch.getLRColumnMapping(lBaseAttrs, rBaseAttrs, JoinIndexBatch.joinCondition.get)
    val compatibleIndexPairs = getCompatibleIndexPairs(
      indexes.get(leftRelation.plan).get,
      indexes.get(rightRelation.plan).get,
      lRMap)
    ...
    compatibleIndexPairs
      .map { indexPairs =>
        val index = JoinIndexRanker
          ...
          .head
        Map(leftRelation.plan -> Seq(index._1), rightRelation.plan -> Seq(index._2))
      }
      .getOrElse(Map.empty)
  }
  ...
  override def reason: String =
    "JoinIndex: no compatible index or " +
      "another index has a higher priority"
}

object JoinIndexBatch extends HyperspaceBatch with HyperspaceEventLogging {

  val checkBatch = JoinNodeCheck :: JoinAttributeCheck :: JoinColumnCheck :: JoinRankCheck :: Nil
  ...
  override def applyIndex(
    ...
    if (!(indexes.size == 2 && indexes.head._2.size == 1 && indexes.last._2.size == 1)) {
      return plan
    }
    plan match {
      case join @ Join(l, r, _, _) =>
        val lIndex = indexes.get(l.collectLeaves().head).get.head
        val rIndex = indexes.get(r.collectLeaves().head).get.head
  ...
  override def score(plan: LogicalPlan, indexes: Map[LogicalPlan, Seq[IndexLogEntry]]): Int = {
    ...
    val leftCandidateIndex = indexes.get(leftRelation.get.plan).get.head
    val rightCandidateIndex = indexes.get(rightRelation.get.plan).get.head
    ...
    val leftCommonBytes = getCommonBytes(leftCandidateIndex, leftRelation.get)
    val rightCommonBytes = getCommonBytes(rightCandidateIndex, rightRelation.get)

    // TODO enhance scoring
    (70 * (leftCommonBytes * 1.0f / leftRelation.get.allFileSizeInBytes)).round +
      (70 * (rightCommonBytes * 1.0f / rightRelation.get.allFileSizeInBytes)).round
  }
